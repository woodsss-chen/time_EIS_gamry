...
function z=I(p,f)% modified inductor
z=p(1)*(j*2*pi*f).^p(2);
end

function z=W(p,f)% Warburg semi-infinite diffusion element
z=sqrt(2)*p(1)./(j*2*pi*f).^(1/2);
end

function z=G(p,f)% Gerischer element (The Gerischer circuit element arises when an electroactive species undergoes a chemical reactionin the bulk.)
z=p(1)./((1+(j*2*pi*f*p(2))).^(1/2));
end

function z = M(p,f)%modified restricted diffusion element
iua=((j*2*pi*f).*p(2)).^(p(3)./2);%iu^alpha/2
z=p(1).*(coth(iua)./iua);
end
